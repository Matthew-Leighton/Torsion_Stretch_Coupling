...
def PlotFig1_Scaled():
	...
	bellfdata = np.array([1,14/16,12/16,11/16])
	bellferror = np.array([0,np.sqrt((14/16)**2 + 1)/16,np.sqrt((12/16)**2 + 1)/16,np.sqrt((11/16)**2 + 1)/16])
	...
	for i in range(len(mean_psi0_squaredlist)):
		ax1.plot(100*epsilon_D_list,psi_D(epsilon_D_list,psi0,mean_psi0_squaredlist[i])/psi0,label=r'$\langle\psi_0^2\rangle =$'+str(mean_psi0_squaredlist[i]),ls=ls_list[i],color=color_list[i],lw=lw_list[i])
	#ax1.errorbar(bellepsilondata,belltwistdata,yerr = psierror,xerr = epsilonerror,color='tab:green',lw=3,zorder=1)
	ax1.errorbar(bellepsilondata,bellfdata,yerr = bellferror,xerr = epsilonerror,color='tab:green',lw=3,zorder=1,ls='none',fmt='o')
	...
	ax2.plot(100*epsilonF_array,DBandStrain_ZetaOne(lambda_array,mean_psi0_squared)/mean_psi0_squared,label=r'$\zeta\to1^+$',lw=1.5,ls = '-.',color='xkcd:purple')
	...
	ax2.plot(100*epsilonF_array,DBandStrain_ZetaInfty(lambda_array,mean_psi0_squared)/mean_psi0_squared,label=r'$\zeta\to\infty$',lw=1.5,ls = ':',color='xkcd:red')

	ax2.errorbar(bell_eTissue_data[0:4],bell_eD_data[0:4]/100 /mean_psi0_squared,yerr = bell_eD_error[0:4]/100 /mean_psi0_squared,xerr = bell_eTissue_error[0:4],color='tab:green',lw=3,zorder=1,ls='none',fmt='o')

	ax2.errorbar(bell_eTissue_data[-1],bell_eD_data[-1]/100 /mean_psi0_squared,yerr = bell_eD_error[-1]/100 /mean_psi0_squared,xerr = bell_eTissue_error[-1],color='tab:green',lw=1,zorder=1,ls='-.',fmt='o',ms=2)
